      ...
      <div className={this.getClass()}>
        <div className="job-name-title">
          ...
          <div className="job-name-title-item-c">
            {this.timeConvert(this.props.job.runTime)}
            {' '}
            /
            {this.timeConvert(this.props.job.timeLimit)}
          </div>
          <div className="job-name-title-item-r">
            {this.props.job.nCpus}
            {' '}
            core
            {(this.props.job.nCpus > 1) ? 's' : ''}
          </div>
        </div>
        <div>
          {this.props.job.name}
          {' '}
          {(this.props.job.nGpus > 0) ? '(GPU)' : ''}
        </div>
      </div>
    );
  }
