...
PARTITIONS = {'skylake-gpu':['john', 'bryan'], 'skylake':['john', 'bryan'], 'sstar':['sstar'], 'gstar':['gstar'], 'knl':['gina']}
CORE_COUNT_NODES = PARTITIONS['skylake']
partitionResGpuCores = {'skylake':4, 'skylake-gpu':4, 'sstar':0, 'gstar':0, 'knl':0}
partitionMaxCpuCores = {'skylake':[32], 'skylake-gpu':[4], 'sstar':[32,16], 'gstar':[32,12], 'knl':[272]}
# arbitrary thresholds for what constitutes a low mem/core or low disk/core situation
partitionLowMem  = {'skylake':1000, 'skylake-gpu':1000, 'sstar':1000, 'gstar':1000, 'knl':200}  # MB per core
partitionLowDisk = {'skylake':3000, 'skylake-gpu':3000, 'sstar':3000, 'gstar':3000, 'knl':600}  #  ""
...
def nodes():
    ...
    for host, n in pyslurm_nodes.items():
        ...
        if debug:
            if host in []:#  'gstar040', 'sstar151', 'john99', 'john2', 'john85', 'john71' ]:
                #print(host, n)
...
def get_core_usage(data):
    ...
    for hostname, node in data['nodes'].items():
        ...
        if debug:
           if hostname in [ "john6", "john99", "gstar102", "sstar011", "sstar107", "sstar301", "gstar201", "bryan1", "gstar040", "gina3", "john32", "john34" ]:
              print(hostname,node)
        ...
        for hostname, node in data['nodes'].items():
            for prefix in prefixes:
                if prefix in hostname:
                        ...
                        if part == "skylake":
                            ...
                            if debug:
                                if s0 + s1 != node['nCpuCores']:
                                    print(hostname, 'err: cpu: ', s0, '+', s1, '!=', node['nCpuCores'], 'node', node)
                                if g0 + g1 != node['nGpuCores']:
                                    print(hostname, 'err: gpu: ', g0, '+', g1, '!=', node['nGpuCores'], 'node', node)
                            ...
                            gpuRes = g0+g1
                            if node['nGpusUsed'] == node['nGpus']: # both gpus being used
                                if gpuRes < 4:
                                    unsched += (4-gpuRes)
                            ...
                            if debug:
                                if cpuCoresIdle0 < 0 or cpuCoresIdle1 < 0:
                                    print('nope, you stuffed up idle', cpuCoresIdle0, cpuCoresIdle1, 'node', node)
                                if cpuCoresIdle0 + cpuCoresIdle1 != nCpuIdleCores:
                                    print(hostname, 'err', cpuCoresIdle0, '+', cpuCoresIdle1, '+', unsched, '!=', nCpuIdleCores, 'node', node)

                            # also need to take account of inaccessible.
                            #     1-4 cores slots for cpu jobs on non-smalljobs, non-largemem nodes will never be used
                            if cpuCoresIdle0 + cpuCoresIdle1 <= 16:   # small jobs could run and would have socket affinity set
                                if not ('smalljobs' in node['features'] or 'largemem' in node['features']):
                                    if cpuCoresIdle0 != 0 and cpuCoresIdle0 <= 4:
                                        unsched += cpuCoresIdle0
                                        cpuCoresIdle0 = 0
                                        if debug:
                                            print('not smalljobs', 'cpuCoresIdle0', cpuCoresIdle0, 'cpuCoresIdle1', cpuCoresIdle1, 'unsched', unsched, hostname, node)
                                    if cpuCoresIdle1 != 0 and cpuCoresIdle1 <= 4:
                                        unsched += cpuCoresIdle1
                                        cpuCoresIdle1 = 0
                                        if debug:
                                            print('not smalljobs', 'cpuCoresIdle1', cpuCoresIdle1, 'cpuCoresIdle0', cpuCoresIdle0, 'unsched', unsched, hostname, node)

                            if debug:
                                if cpuCoresIdle0 + cpuCoresIdle1 + unsched < nCpuIdleCores:
                                    print(hostname, 'err2', cpuCoresIdle0, '+', cpuCoresIdle1, '+', unsched, '<', nCpuIdleCores, 'node', node)
                            ...
                            if debug and node['nCpus'] - node['nCpusUsed'] - (cpuCoresIdle0 + cpuCoresIdle1 + unsched) != 0:
                                print('unsched', 'free - (c0+c1+un)', node['nCpus'] - node['nCpusUsed'] - (cpuCoresIdle0 + cpuCoresIdle1 + unsched), 'cpuCoresIdle0', cpuCoresIdle0, 'cpuCoresIdle1', cpuCoresIdle1, 'unsched', unsched, hostname, node)
                            ...
                            if node['nCpus'] - node['nCpusUsed'] > 0:
                                ...
                                if debug:
                                    if part == 'skylake-gpu' and node['nGpus'] - node['nGpusUsed'] > 0:
                                        print('blocked gpu', node['nGpus'] - node['nGpusUsed'], node)
                    ...
                    if debug:
                        if node['nCpus'] == 0 and node['state'] != 'IDLE' and node['avail']:
                            print('state not idle or unavail, yet 0 cores used', hostname, node)
                        if node['nGpusUsed'] > 0 and node['nGpuCores'] == 0 or node['nGpusUsed'] == 0 and node['nGpuCores'] > 0:
...
def usage(n):
    if n == 'qinfo':
        ...
        print("   queue partitioning, socket affinity, ram, etc. so is mostly accurate.")
    else:
...
if __name__ == '__main__':
    ...
    if n == 'spart.py':
        for k in [ "skylake", "skylake-gpu", "sstar", "gstar", "knl" ]:
            ...
            if k in [ "skylake", "knl" ]:
                ...
                print('%12s' % k, 'idle gpus', u[k]['gpus']['i'], 'used', str(int(perc['gpus'])) + '%',
                         '(idle cores %d%% idle nodes %d%%)' % ( int(perc['cores']), int(perc['nodes']) ))
        ...
        if len(sys.argv) > 1 and sys.argv[1] == '-s':
            ...
            cw['queue']=0
            for k in [ "partition", "queue/", "skylake", "skylake-gpu", "sstar", "gstar", "knl" ]:
