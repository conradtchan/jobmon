...
class GPUmapping:
    ...
    def update_jobs(self, jobs):
        ...
        for jid in jobs:
            ...
            if job["state"] == "RUNNING":
                if (
                    job["nGpus"] > 0 and len(job["layout"].keys()) == 1
                ):  # For now, ignore multi-node GPU jobs
        ...
        for jid in list(self.unknown.keys()):
            ...
            if delete:
                ...
                del_count += 1
        if del_count > 0:
            print("Deleted {:} known GPU mappings".format(del_count))
        ...
        for jid in list(self.mapping.keys()):
            ...
            if delete:
                ...
                del_count += 1
        if del_count > 0:
            print("Deleted {:} unknown GPU mappings".format(del_count))
    ...
    def update_node_info(self):
        ...
        for host in pyslurm_node:
            ...
            if gpu_string.split(":")[1] != "0":
                ...
                if gpus_used > 0:
                    gpu_mapping[host] = [
                        int(x) for x in gpu_string.split(":")[3].strip(")").split("-")
                    ]
    ...
    def determine(self):
        ...
        print(
            "Determined GPU mappings: ",
            len(self.mapping),
            "/",
            len(self.mapping) + len(self.unknown),
        )
